...
class Auth_model extends CI_Model {
    ...
    public function checkId($identity){
        $query = $this->db->select($this->identity_column . ',id, licence, expired_date, group_id')
        ...
        if ($query->num_rows() === 1) {
            ...
            if($row['licence'] != "" && strtotime(base64_decode($row['expired_date'])) > strtotime(date("Y-m-d"))) {
                $this->db->where($this->identity_column, $this->db->escape_str($identity))->update("tec_users",array("licence"=>"", "expired_date"=>""));
                $row['licence'] = "";
                $row['expired_date'] = "";
            }
            return $rows;
    ...
    public function login($identity, $password, $remember = FALSE) {
        ...
        if ($query->num_rows() === 1) {
            ...
            if ($password === TRUE) {
                ...
                $rows = get_rows("user_logins",array("user_id"=>$user->id));
                if(count($rows) == 0) $this->session->set_userdata("change_password_status","yes");
