...
class Pos_model extends CI_Model {
        ...
        if ($this->db->insert('sales', $data)) {
            ...
            foreach ($items as $item) {
                ...
                if ($this->db->insert('sale_items', $item)) {
                   ...
                   # if ($product->cost > 0) {
                   #     $this->db->insert('purchases', ["note" => "Custo do produto {$product->name}", "total" => $product->cost * $item['quantity']]);
                  #  }
            ...
            if (is_array($_POST["subitem_id"])) {
                foreach ($_POST["subitem_id"] as $k => $v) {
                    ...
                    $q = new Query();
                    $q
                            ->insert("tec_sale_sub_items", array(
                                "sale_id" => $sale_id,
                                "subitem_id" => $v,
                                "quantity" => $_POST["quantity_item"][$k],
                                "subtotal" => $prod['price'] * $_POST["quantity_item"][$k],
                                "real_unit_price" => $prod['price']
                            ))
                            ->run();
                    if (!$q) {
                        die("error !");
            ...
            if ($did) {
                ...
                $this->db->delete('suspended_subitems', array('suspend_id' => $did));
            }
            $msg = array();
            if (!empty($payment)) {
                if ($payment['paid_by'] == 'stripe') {
                    ...
                    unset($payment['cc_cvv2']);
                    $payment['sale_id'] = $sale_id;
                    $payment['OrderReferences'] = $data['InvoiceNo'];
    ...
    public function suspendSale($data, $items, $did = NULL) {
        $mesa = Func::array_table("tec_mesas", ["name" => $data["hold_ref"]]);
        ...
        if (is_array($mesa)) {
            $q = new Query();
            $q
                    ->update("tec_mesas", array("status" => 1))
                    ->where_equal(array("id" => $mesa["id"]))
                    ->run();
        ...
        #  exit();
        if ($did) {
            if ($this->db->update('suspended_sales', $data, array('id' => $did)) && $this->db->delete('suspended_items', array('suspend_id' => $did)) && $this->db->delete('suspended_subitems', array('suspend_id' => $did))) {
                foreach ($items as $item) {
                    unset($item['cost']);
                    $item['suspend_id'] = $did;
                    $this->insertInto('tec_suspended_items', $item);
                }
                $this->suspenseSubitens($did);
                return TRUE;
            }
        } else {
            if ($this->db->insert('suspended_sales', $data)) {
                ...
                foreach ($items as $item) {
                    ...
                    $item['tax'] = Func::extractOnlyNumbers($item['tax']);
                    if (empty($item['tax'])) {
                        $item['tax'] = 0;
                    }
                    $this->insertInto('tec_suspended_items', $item);
                }
                $this->suspenseSubitens($suspend_id);
                return $suspend_id;
            }
            return true;
        }
    ...
    private function suspenseSubitens($suspend_id) {
        $suspenseSubitens = $_POST['subitem_id'];
        if (is_array($suspenseSubitens) && count($suspenseSubitens) > 0) {
            foreach ($suspenseSubitens as $k => $subitem) {
                $subProduct = Func::array_table("tec_producs_list", array("id" => $subitem));
                $this->insertInto('tec_suspended_subitems', array(
                    "suspend_id" => $suspend_id,
                    "product" => $subProduct["product"],
                    "sub_product_id" => $subitem,
                    "quantity" => $_POST["quantity_item"][$k],
                    "unit_price" => $subProduct["price"],
                    "subtotal" => $subProduct["price"] * $_POST["quantity_item"][$k],
                ));
            }
        }
    }

    private function insertInto($table, $set) {
        $q = new Query();
        $q
                ->insert($table, $set)
                ->run();
    ...
    public function getAllSaleItems($sale_id) {
        $this->db->select('sale_items.*, products.name as product_name, products.id as product_id, products.code as product_code, products.tax_method as tax_method')
    ...
    public function getSuspendedSaleSubItems($id) {
        $q = $this->db->get_where('suspended_subitems', array('suspend_id' => $id));
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
