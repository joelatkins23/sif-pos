...
class Products extends MY_Controller {
    ...
    function itens() {
        $data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
        $this->data['page_title'] = lang('products');
        $bc = array(array('link' => '#', 'page' => lang('products_item')));
        $meta = array('page_title' => lang('products_item'), 'bc' => $bc);
        $this->page_construct('products/Itens', $this->data, $meta);
    }

    function update_item_form() {
        $q = new Query();
        $q
                ->update("tec_producs_list", array(
                    "name" => $_POST["name"],
                    "price" => $_POST["price"],
                ...
                ->run();
        $this->session->set_flashdata('message', "Adicional alterada com sucesso");
        redirect('products/itens');
    }

    function add_item_form() {
        if (empty($_POST["product"])) {
            $this->session->set_flashdata('error', "Selecione um produto");
            redirect('products/add_item');
        }
        $q = new Query();
        $q
                ->insert("tec_producs_list", array(
                    "product" => $_POST["product"],
                    "name" => $_POST["name"],
                    "price" => $_POST["price"]
                ))
                ->run();
        if ($q) {
            $this->session->set_flashdata('message', "Novo adicional, adicionado com sucesso");
            redirect('products/itens');
        }
        $this->session->set_flashdata('error', "Selecione um produto");
        redirect('products/add_item');
    ...
    function get_products_itens() {
        ...
        $q  ->select(
                ...
                ->inner_join("tec_products ON tec_producs_list.product = tec_products.id")
                ->run();
        $row = array();
        ...
        foreach ($q->get_selected() as $data) {
            $row[] = array(
                ...
                $data["name"],
                $data["price"],
                "<a href='" . site_url("products/edit_item/{$data["id"]}") . "' title='" . lang("edit_product") . "' class='tip btn btn-warning btn-xs'><i class='fa fa-edit'></i></a> "
                . "<a href='" . site_url("products/delete_item/{$data["id"]}") . "' onClick=\"return confirm('" . lang('alert_x_product') . "')\" title='" . lang("delete_product") . "' class='tip btn btn-danger btn-xs'><i class='fa fa-trash-o'></i></a></div></div>",
                "pid, image, code, pname"
            );
        }
        $output['aaData'][] = $row;
        $output = array(
    ...
    function delete_item($id = NULL) {
        ...
        $q
                ...
                ->run();
        $this->session->set_flashdata('message', "Adicional deletado com sucesso");
        redirect('products/itens');
    }
    function get_products() {
        ...
        $product_amount = $this->input->get("product_amount")?$this->input->get("product_amount"):NULL;
        $where = array();
        if($category_id != NULL && $category_id != "-1"){
            $this->datatables->where('tec_products.category_id', $category_id); 
        }
        if($product_amount != NULL){
            $this->datatables->where('tec_products.quantity <=', $product_amount); 
        }
    ...
    function print_products(){
        ...
        $product_amount = $this->input->post('product_amount') ? $this->input->post('product_amount') : NULL;
        $where_in = array();
        $where = array();
        if($category_id != NULL && $category_id != "-1"){
            $where['tec_products.category_id'] = $category_id;
        }

        if ($product_amount) {
            $where['tec_products.quantity<='] = $product_amount;
        }
        $products = get_rows_join("tec_products",$where," tec_products.name ASC",array(), "tec_products.*, tec_categories.name as category_name",   $where_in,array(array("table"=>"tec_categories","where"=>"tec_products.category_id=tec_categories.id")));

        $this->load->view($this->theme . 'products/print_products', array("products"=>$products));
    ...
    function add_itens() {
        ...
        $meta = array('page_title' => "Adicionar Produto adicional", 'bc' => $bc);
        $this->data['page_title'] = "Produto adicional";
        $this->page_construct('products/add_item', $this->data, $meta);
    }
    ...
    function edit_item($id = NULL) {
        ...
        $q
                ->select()
                ->from("tec_producs_list")
                ->where_equal(array("id" => $id))
                ->limit(1)
                ->run();
        $this->data['data'] = $q->get_selected();
        $bc = array(array('link' => site_url('products'), 'page' => lang('products')), array('link' => '#', 'page' => lang('edit_product')));
        $meta = array('page_title' => "Alterar Adicional do produto", 'bc' => $bc);
        $this->page_construct('products/update_item', $this->data, $meta);
    }
    ...
    function edit($id = NULL) {
        ...
        $this->form_validation->set_rules('price', lang("product_price"), 'required|is_numeric');
        $this->form_validation->set_rules('cost', lang("product_cost"), 'required|is_numeric');
        $this->form_validation->set_rules('tax_id', lang("tax_id"), 'required|is_numeric');
        $this->form_validation->set_rules('quantity', lang("quantity"), 'is_numeric');
        $this->form_validation->set_rules('alert_quantity', lang("alert_quantity"), 'is_numeric');
