        ...
        <table align="center">
            <tr>
                ...
                <?= $Settings->tel; ?>
                                      ...
                                      <td><b style="font-size: 20px !important;">TALÃO DE CONTROL</b></td>
                                    </tr>
                                </table>
                                ...
                                <p>&nbsp;</p>
                                <table border="1" id="totalTable" style="width: 100%; float: right; padding: 5px; color: #000000; background: #FFF;">
                                    <tbody>
                                        <tr>
                                            <td style="border-left: 1px solid #ddd; border: 1px solid #333; padding-left:10px; text-align:left; font-weight:normal;">
                                                <?= lang('total_items'); ?>
                                            </td>
                                            <td style="text-align:right; padding-right:10px; border: 1px solid #333; font-weight:bold;"><span id="count">0</span></td>
                                            <td style="padding-left:10px; border: 1px solid #333; text-align:left;"><?= lang('total'); ?></td>
                                            <td style="border: 1px solid #333; text-align:right; padding-right:10px; font-weight:bold;">
                                                <span id="total">.00</span></td>
                                        </tr>
                                        <tr>
                                            <td style="border-left: 1px solid #ddd;border: 1px solid #333; padding-left:10px; text-align:left; "><?= lang('order_discount'); ?></td>
                                            <td style="text-align:right; padding-right:10px;border: 1px solid #333; font-weight:bold;">
                                                <span id="ds_con">0.00</span></td>
                                            <td style="padding-left:10px; text-align:left;border: 1px solid #333; font-weight:normal;">Caixa</td>
                                            <td style="border-right: 1px solid #ddd; text-align:right; border: 1px solid #333;padding-right:10px; font-weight:bold;"><?= $this->session->userdata('username'); ?></td>
                                        </tr>
                                        <tr>
                                            <td colspan="2"
                                                style="padding: 5px 0px 5px 10px; text-align: left; border: 1px solid #333; font-size: 16px; border: 1px solid #333; font-weight: bold; background: #333; color: #FFF;">
                                                    <?= lang('total_payable'); ?>
                                            </td>
                                            <td colspan="1"
                                                style="text-align: right; padding: 5px 10px 5px 0px; font-size: 16px; border: 1px solid #333; font-weight: bold; background: #333; color: #FFF;">
                                                <span id="total-payable">0.00</span></td>
                                            <td colspan="1"
                                                style="text-align: right; padding: 5px 10px 5px 0px; font-size: 16px; border: 1px solid #333; font-weight: bold; background: #333; color: #FFF; font-size:12px ">
                                                <span id="table">0.00</span></td>
...
<?php unset($Settings->protocol, $Settings->smtp_host, $Settings->smtp_user, $Settings->smtp_pass, $Settings->smtp_port, $Settings->smtp_crypto, $Settings->mailpath, $Settings->timezone, $Settings->setting_id, $Settings->default_email, $Settings->version, $Settings->stripe, $Settings->stripe_secret_key, $Settings->stripe_publishable_key); ?>
        ...
        <script type="text/javascript">
            ...
            function loadItems() {
                ...
                var $default = localStorage.getItem('table-t');
                if ($default.length > 0) {
                    $('#table').html("Para: " + $default);
                    ...
                    if (sposSubitems !== null && sposSubitems.length !== 0) {
                        $.each(spositems, function () {
                            ...
                            $.post(base_url + 'ajax/parseSubItem.php', {item: sposSubitems}, function (json) {
                                ...
                                $("#billTable tbody").append(newTr.html(tr_html));
                                ...
                                $.each(json.item, function (k, v) {
                                    if (k === item_id) {
                                        ...
                                        $.each(v, function (kk, vv) {
                                            //console.log(vv);
                                            console.log("caiu 1 => " + total);
                                            //  $adicional += parseFloat(vv.value) /** formatDecimal(vv.count)*/;
                                            total += parseFloat(vv.value) * formatDecimal(vv.count);
                                            console.log("caiu 2 => " + total);
                                            count += parseFloat(vv.count);
                                            ...
                                            // console.log(parseFloat(vv.value) * formatDecimal(vv.count));
                                            $tr_html = '<td> → ' + vv.name + '</td>';
                                            $tr_html += '<td class="text-right"> ' + formatMoney(parseFloat(vv.value)) + '</td>';
                                            $tr_html += '<td class="text-center">' + formatDecimal(vv.count) + '</td>';
                                            $tr_html += '<td class="text-right">' + formatMoney(((parseFloat(vv.value)) * parseFloat(vv.count))) + '</td>';
                                            $("#billTable tbody").append("<tr>" + $tr_html + "</tr>");
