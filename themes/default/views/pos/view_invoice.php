    ...
    <div class="no-print">
        <?php if ($message) { ?>
            <div class="alert alert-success">
                <a  href="<?= site_url('pos/cash_sale'); ?>" class="btn btn-success"> <i class="glyphicon glyphicon-plus"></i> Voltar</a><button data-dismiss="alert" class="close" type="button">×</button>
                <?= is_array($message) ? print_r($message, true) : $message; ?>
            </div>
        <?php } ?>
    </div>
    <div class="cancelado">
        <img src="<?php echo $assets; ?>images/header/principal2.png" alt="Logotipo"  width="500" height="250" border="0" />
    </div>
    <div  id="receipt-data" >
        ...
        <div class="text-center" style="font-size: 12px;" >
            <table style="width: 100%">
                <tr>
                        ...
                        <table style="width: 100%;">
                            <tr>
                                <td width="50%" style="text-align: left; padding-right: 20px;">
                                    <img src="<?php echo $assets; ?>images/header/principal2.png" alt="Logotipo"  width="100" height="105" border="0" />
                                </td>
                            </tr>
                            <tr>
                    ...
                    <td width="50%" style="text-align: left; padding-left: 10%; vertical-align: top; padding-top: 15px;">
                      ...
                      <p class="print_color"  style="font-size: 30px;  text-align: left; ">
                            <?php
            ...
            <table style="width: 100%; margin: 25px 0px;">
                <tr style="border-bottom: 1px solid black;">
                    ...
                    <td style="text-align: right;">
                        <span class="print_order">
                            <?php
                                if($this->session->userdata("invoice_status") == "reprint") echo "2Via";
                                else echo "ORIGINAL";
                            ?>
                        </span>
                    </td>
            ...
            <div class="print-spec">
                <table width="98%" height="45" class="table table-striped table-condensed" style="margin-top: 5px;">
                    ...
                    foreach ($rows as $row) {
                        $ok++;
                        if(count($rows)>=14 && $ok % 30 == 0){

                            // echo "<tr style='border:0px;'><td>OKOK</td></tr>";
                            echo '</table>'
                            ?>
                            <p class="no-screen-p print_color"  style="font-size: 36px;  text-align: right; ">
                            <?php
                                switch ($inv->InvoiceType) {
                                    case 'FR':
                                        echo lang("invoice_receipt");
                      ...
                      <p class="no-screen text-align-right" style="font-size: 16px; text-align: right !important;">
                       <b style="text-align: right;"> <?= $inv->InvoiceNo;?></b>
                        </p>
                      <table style="width: 100%; margin: 25px 0px;" class="no-screen">
                        <tr style="border-bottom: 1px solid black;">
                            <td style="width: 40%; text-align: left;">
                                <?php 
                                    echo lang("issuance_date").": <b>".$this->tec->hrld($inv->date)."</b>";
                        ...
                        echo '<tr><td  style="font-size: 10px" width="5%" align="left">' . product_name($row->product_code). '</td>';
                        if($item_code  != "")
                        ...
                        echo '<td style="font-size: 10px" width="9%" class="text-right">' . $this->tec->formatNumber($row->quantity) . '</td>';
                        echo '<td style="font-size: 10px" width="15%" class="text-right">';
                        ...
                        echo '<td style="font-size: 10px" width="10%" class="text-right">' . $this->tec->formatNumber($row->item_tax) . '</td>';
                        echo '<td style="font-size: 10px" width="9%" class="text-right">' . $this->tec->formatNumber($row->item_discount) . '</td>';

                        echo '<td style="font-size: 10px" width="10%" class="text-right">' . $this->tec->formatMoney($row->subtotal) . '</td></tr>'; 
                    }
                    $no = 0;
                    if(count($rows)>14) {
                        if(count($rows)>30){
                            ...
                            for($i=1; $i<=35-$aa; $i++){
                                $no++;
                                echo '<tr class="no-screen-tr" style="border:0px;"><td>&nbsp;</td></tr>';
                            }
                        } else {
                            $aa = count($rows) % 25;
                            for($i=1; $i<=28-$aa; $i++){
          ...
          <?php
          if($no != 0)      { ?>
                      <p class="no-screen-p print_color"  style="font-size: 26px;  text-align: right; ">
                            <?php
                                switch ($inv->InvoiceType) {
                                    case 'FR':
                                        echo lang("invoice_receipt");
                      ...
                      <p class="no-screen text-align-right" style="font-size: 16px; text-align: right;">
                           <b> <?= $inv->InvoiceNo;?></b>
                        </p>
                      <table style="width: 100%; margin: 25px 0px;" class="no-screen">
                        <tr style="border-bottom: 1px solid black;">
            ...
            // print_r($payments)
                            if ($payments) {
                                ...
                                foreach ($payments as $payment) {
                                    ...
                                    if ($payment->paid_by == 'OS' && $payment->amount) {
                                        echo '<td>' . lang("paid_by") . ': ' . lang($payment->paid_by) . '</td>';
                                        echo '<td>' . lang("amount") . ': ' . $this->tec->formatMoney($payment->pos_paid == 0 ? $payment->amount : $payment->pos_paid) . '</td>';
                ...
                <?= $Settings->footer; ?>
                   ...
                   <p style="text-align: left;font-size: 12px;padding-bottom: 10px;">
                            <?php
                                $hash_str = $inv->Hash;
                            ?>
                        <?php echo substr($hash_str, 0,1).substr($hash_str, 10,1).substr($hash_str, 20,1).substr($hash_str, 30,1) ; ?>-Processado por programa validado nº 230/AGT/2019 - SIF POS</p>     
                  </td>
                    <td style="width: 40%; padding-left: 20px; vertical-align: top;">
                        <table style="width: 100%; font-size: 12px;" class="total_table">
                            ...
                            if ($inv->total_discount != 0) {
                                echo '<tr > <th style="font-size: 12px">' . lang("order_discount") . '</th><th class="text-right" style="font-size: 12px">' . $this->tec->formatMoney($inv->total_discount) . '</th></tr>';
                            }
                            ...
                            ?>
                                <tr>
                                    <th height="2" style="font-size: 12px" ><?= lang("rounding"); ?></th>
                                    <th  class="text-right" style="font-size: 12px"><?= $rounding; ?></th>
                                </tr>
                                <tr>
                            ...
                            <?php
                                ...
                                ?>
                                <tr>
                                    <th style="padding: 0px;"><p class="td-color" style="font-size: 12px"><?= lang("grand_total"); ?></p></th>
                                    <th style="padding: 0px;"><p class="td-color" style="text-align: right; font-size: 12px;"><?= $this->tec->formatMoney($inv->grand_total); ?></p></th>
                                </tr>
                            <?php }
        ...
        <?php if (!$Settings->java_applet && !$noprint) { ?>
            $(function(){
                ...
                $(".page-number").each(function(){
                    ok++; 
                    $(this).html("Pag ." + ok + " / " + $(".page-number").length);
                })
                window.print();
