    ...
    function daily_sales($year = NULL, $month = NULL)
    {
        ...
        if(!empty($sales)) {
            foreach($sales as $sale){
                $daily_sale[$sale->date] = "<span class='text-warning'>". formatMoney( $sale->tax)."</span><br>".$sale->discount."<br><span class='text-success'>".formatMoney($sale->total)."</span><br><span style='border-top:1px solid #DDD;'>".formatMoney($sale->grand_total)."</span>";
            }
        } else {
            $daily_sale = array();
        }
