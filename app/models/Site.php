...
class Site extends CI_Model {
    ...
    public function getQtyAlerts() {
        /*
          ...
          # ->from('products')
          ->where('quantity < alert_quantity OR quantity < 1', NULL, FALSE)
          ->group_by('products.id');
          return $this->db->count_all_results('products'); */

        $this->db->where('quantity < alert_quantity OR quantity < 1', NULL, FALSE)
                ->join('categories', 'categories.id=products.category_id');
        #->group_by('products.id');
        return $this->db->count_all_results('products');
    }
    ...
    public function getAllUsers() {
        $this->db->select("users.id as id, first_name, last_name, email, company, active, group_id")
                ->group_by('users.id');
        $q = $this->db->get('users');
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $group_id = base64_decode($row->group_id);
                $this->db->where("id",$group_id);
                $row1 = $this->db->get("groups")->row_array();
                $row->group = $row1['name'];
                $data[] = $row;
            }
            return $data;
    ...
    public function getSubProductByID($id) {
        $q = $this->db->get_where('producs_list', array('id' => $id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
    ...
    public function getUserGroup($user_id = NULL) {
        if ($group_id = base64_decode($this->getUserGroupID($user_id))) {
